from datetime import timedelta, datetime


def parse_duration(duration_str):
    
    duration_str = duration_str.replace("P", "").replace("T", "")

    components = ['D', 'H', 'M', 'S']
    values = {'D': 0, 'H': 0, 'M': 0, 'S': 0}

    for component in components:
        if component in duration_str:
            value, duration_str = duration_str.split(component)
            values[component] = int(value)

    total_duration = timedelta(
        days=values['D'],
        hours=values['H'],
        minutes=values['M'],
        seconds=values['S']
    )

    return total_duration.total_seconds()



# def transform_data(row):

#     duration_td = parse_duration(row['duration'])
#     row['duration'] = (datetime.min + duration_td).time()
#     row['Video_Type'] = 'Shorts' if duration_td.total_seconds() <= 60 else 'Normal'

#     return row


def transform_data(row):
    # Get total seconds as float
    seconds = parse_duration(row['duration'])
    
    # Create actual timedelta object for math
    td = timedelta(seconds=seconds)
    
    # Update row
    row['duration'] = (datetime.min + td).time().strftime('%H:%M:%S')
    row['video_type'] = 'Shorts' if seconds <= 60 else 'Normal'

    return row

