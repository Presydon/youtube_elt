...
def staging_table():
    ...
    try:
        ...
        logger.info(f"Staging table updated successfully with {len(YT_data)} records.")
...
def core_table():
    ...
    try: 
        ...
        ids_to_delete = set(table_ids) - current_videos_ids
        if ids_to_delete:
            delete_rows(cur, conn, schema, ids_to_delete)

        logger.info(f"Core table updated successfully with {len(rows)} records.")
